...
#### 第四轮:补 bench 对比默认后端,发现 IOCP 真异步路径是死代码

补 bench 对比 TokioFile vs 默认后端(Windows IOCP / Linux io_uring),
三后端完整模式(100 样本)数据:

| bench | memory | tokio_file | default_io_strategy |
|-------|--------|------------|---------------------|
| disk_io_backends(4MiB/4分片) | 20.45ms [18.79,21.18] | 30.30ms [28.68,31.49] | 28.93ms [25.76,32.32] |
| large_file_fragmented(16MiB/16分片) | 40.48ms [34.64,46.99] | 70.86ms [66.30,74.47] | 69.96ms [63.05,79.40] |

**关键发现:IOCP(默认后端)与 TokioFile 性能完全相同**(CI 重叠),无任何优势。

**根本原因:IOCP 真异步路径是死代码**。Agent 逐行验证完整调用链:

1. bench 的 chunk 数据源自 `Vec<u8>` 堆分配(`bench_server.rs:257,263`),
   或 downloader 的 `BytesMut::with_capacity`(`downloader.rs:1334`),
   两者堆分配仅保证 16 字节对齐
2. `IoCpStorage::write_at`(`iocp.rs:997-1000`)检查 buffer 指针是否 512 对齐:
   `!buf_addr.is_multiple_of(Self::IOCP_SECTOR_SIZE)` -> **几乎必然 false**
3. `needs_fallback = true` -> 所有写入走 fallback(`iocp.rs:1001-1017`)
4. fallback = `spawn_blocking` + `seek_write` + per-file `write_lock` Mutex 串行化
5. TokioFile 的 `write_at`(`tokio_file.rs:85-95`)= `spawn_blocking` + `seek_write` + `write_lock`
6. **两者结构等价,性能必然相同**

`submit_iocp_write`(`iocp.rs:533-639`)的真异步 OVERLAPPED 路径(无锁、真并行、
poller 批量收完成事件)在 bench 场景下**从未被触发**。这影响所有 Windows 用户--
默认 IOCP 后端从未真正使用过真异步写入,退化成 TokioFile 等价物。

**同样的指针对齐问题也影响 WinFile**(`winio.rs:238-239` 注释明确提及)和
