...
pub struct MagnetConfig {
    ...
    /// 跳过 tracker/DHT 等待直接连接已知 peer,加速冷启动。
    #[serde(default)]
...
#### 4.3.4 initial_peers 来源

`AddTorrentOptions.initial_peers: Option<Vec<SocketAddr>>`(session.rs:268,源码确认存在)
接受预置 peer 地址列表。来源:

1. **磁力链接 `&pe=` 参数解析**:BEP 9 规定 magnet URI 可含 `pe` 参数(peer 地址)。
   `MagnetProtocol::probe` 解析 `&pe=host:port` 注入 `initial_peers`。
2. **用户手动配置**:`MagnetConfig.peer_addrs: Vec<String>` 允许用户为特定资源
   预置已知 peer 地址(如自建 seed 节点)。

**注意**:`export_peer_addrs()`(原设计从 `per_peer_stats_snapshot` 导出活跃 peer)
无法实现——`PeerStatsFilter`/`PeerStatsSnapshot` 在 librqbit 8.1.1 未公开 re-export
(`torrent_state` 是私有 `mod`)。因此 initial_peers 持久化(任务结束导出 peer 列表)
本期不做,改为静态来源(magnet `&pe=` 参数 + 用户配置)。

后续若 librqbit 升级公开 `PeerStatsFilter`,可补 peer 持久化导出。
...
impl MagnetProtocol {
    ...
    /// - `live.stats_snapshot() -> StatsSnapshot`(live/stats/snapshot.rs:8)
    /// - `StatsSnapshot.peer_stats: AggregatePeerStats`(peers/stats/snapshot.rs:8,
    ///   含 `queued`/`connecting`/`live: usize`)
    /// - `StatsSnapshot.downloaded_and_checked_bytes/uploaded_bytes: u64`
    ...
    // 注:export_peer_addrs() 已移除 —— PeerStatsFilter/PeerStatsSnapshot 在
    // librqbit 8.1.1 未公开 re-export(torrent_state 是私有 mod),无法调
    // per_peer_stats_snapshot。initial_peers 改用 magnet &pe= 参数 + 用户配置。
}
...
| `test_parse_pe_param_from_magnet_url` | 解析 magnet `&pe=host:port` 为 initial_peers |
