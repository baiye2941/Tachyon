...
beforeEach(async () => {
  ...
  historyStore = {
    ...
    deleteHistoryRecord: mod.deleteHistoryRecord,
  };
});
...
describe("HistoryStore 历史记录存储", () => {
  ...
  it("根据 ID 删除记录并更新 localStorage", () => {
    ...
    const removeId = historyStore.records.find(
      (r) => r.fileName === "remove.zip",
    )!.id;
    historyStore.deleteHistoryRecord(removeId);

    expect(historyStore.records).toHaveLength(1);
    expect(historyStore.records[0]?.fileName).toBe("keep.zip");

    const stored = JSON.parse(localStorage.getItem(STORAGE_KEY)!);
    expect(stored).toHaveLength(1);
    expect(stored[0].fileName).toBe("keep.zip");

    // 删除不存在的 ID 应为 no-op
    historyStore.deleteHistoryRecord("non-existent");
    expect(historyStore.records).toHaveLength(1);
  });
});
...
describe("HistoryStore 过滤功能", () => {
  beforeEach(() => {
    const statuses: HistoryRecord["status"][] = [
      "completed",
      "failed",
      "cancelled",
      "completed",
...
describe("HistoryStore 统计计算", () => {
  ...
  it("统计计算准确", () => {
    ...
    expect(stats.totalBytes).toBe(
      1024 * 1024 + 512 * 1024 + 2048 * 1024 + 256 * 1024,
    );
    expect(stats.avgSpeed).toBeCloseTo(
      (204800 + 256000 + 204800 + 100000) / 4,
      0,
    );
