...
describe("Accessibility Tests", () => {
  ...
  describe("触摸目标尺寸（≥44px）", () => {
    ...
    it("TaskItem 应该有足够的触摸目标尺寸", () => {
      ...
      const taskElement = expectElement(
  ...
  describe("TitleBar ARIA 标签", () => {
    it("最小化按钮应该有 aria-label", () => {
      const { container } = render(() => <TitleBar />);
      const minimizeBtn = expectElement(
        container.querySelector('[aria-label="最小化窗口"]'),
      );
    ...
    it("最大化/恢复按钮应该有动态 aria-label 和 title", () => {
      const { container } = render(() => <TitleBar />);
      const maximizeBtn = expectElement(
        container.querySelector('[aria-label="最大化窗口"]'),
      );
    ...
    it("关闭按钮应该有 aria-label 和 title", () => {
      const { container } = render(() => <TitleBar />);
      const closeBtn = expectElement(
        container.querySelector('[aria-label="关闭窗口"]'),
      );
  ...
  describe("TaskItem 键盘导航", () => {
    ...
    it("应该响应 Enter 键触发 onClick", () => {
      ...
      fireEvent.keyDown(
        expectElement(container.querySelector<HTMLElement>('[role="button"]')),
        { key: "Enter" },
      );
