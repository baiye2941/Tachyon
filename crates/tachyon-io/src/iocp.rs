...
mod tests {
    ...
    /// C-01 回归测试:`read_at` 跨 `spawn_blocking` 的 Use-After-Free(IoCpStorage)。
    ///
    /// 复现 downloader.rs verify 取消路径:JoinSet 并发 read_at + abort_all。
    /// IoCpStorage::read_at 走 buffered fallback 句柄的 seek_read,旧实现将
    /// `&mut [u8]` 裸指针 move 进 spawn_blocking,abort 时 buf 释放但阻塞任务
    /// 持悬垂指针 → UAF。修复后闭包内分配 owned Vec 读盘,await 后写回,取消安全。
    #[cfg(target_os = "windows")]
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    async fn test_iocp_read_at_joinset_abort_all_no_uaf() {
        use crate::storage::AsyncStorage;
        use tokio::task::JoinSet;

        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("iocp_c01_abort_read.dat");
        let mut storage = IoCpStorage::new(&path);
        storage.init().expect("IOCP init 应成功");
