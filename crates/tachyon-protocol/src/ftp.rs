...
impl FtpClient {
    ...
    pub async fn connect(&self, host: &str, port: u16) -> DownloadResult<()> {
        let normalized_host = host.trim_end_matches('.');
        if normalized_host.eq_ignore_ascii_case("localhost") {
            return Err(DownloadError::Protocol("FTP 不允许访问 localhost".into()));
        }
    ...
    /// 必须在 `connect()` 成功后调用。
    pub async fn login(&self, user: &str, pass: &str) -> DownloadResult<()> {
        let mut guard = self.inner.write().await;
        ...
        stream
            .login(user, pass)
            .await
            .map_err(|e| DownloadError::Protocol(format!("FTP 登录失败: {e}")))?;
    ...
    /// 必须在 `login()` 成功后调用。
    pub async fn file_size(&self, path: &str) -> DownloadResult<u64> {
        let mut guard = self.inner.write().await;
        ...
        let size = stream
            .size(path)
            .await
            .map_err(|e| DownloadError::Protocol(format!("获取文件大小失败: {e}")))?;
        Ok(size as u64)
    }
    ...
    /// 必须在 `login()` 成功后调用。
    pub async fn retrieve_range(&self, path: &str, start: u64, end: u64) -> DownloadResult<Bytes> {
        let mut guard = self.inner.write().await;
        Self::require_stream(&guard)?;
        let stream = guard
            .stream
        ...
        let need = if end >= start {
            (end - start + 1) as usize
        } else {
            return Err(DownloadError::Protocol(
                "FTP Range 无效: end < start".into(),
            ));
        };
    ...
    fn require_stream(state: &ConnectionState) -> DownloadResult<()> {
    ...
    ///
    /// - 用户名默认 `"anonymous"`
    /// - 密码默认 `"anonymous@aimodeldownloader"`
    /// - 端口默认 `21`
    fn parse_ftp_url(url: &str) -> DownloadResult<FtpUrl> {
        let parsed = url::Url::parse(url)
            .map_err(|e| DownloadError::Protocol(format!("FTP URL 解析失败: {e}")))?;
        ...
        match parsed.host() {
            Some(url::Host::Ipv4(v4)) => {
                reject_forbidden_ip(IpAddr::V4(v4))
                    .map_err(|e| DownloadError::Protocol(e.to_string()))?;
            }
            ...
            None => {
                return Err(DownloadError::Protocol("FTP URL 缺少主机名".into()));
            }
        }

        let host = parsed
            .host_str()
            .ok_or_else(|| DownloadError::Protocol("FTP URL 缺少主机名".into()))?
            .to_string();
        ...
        let password = parsed
            .password()
            .unwrap_or("anonymous@aimodeldownloader")
            .to_string();
...
mod tests {
    ...
    async fn test_ftp_url_parsing_defaults() {
        ...
        assert_eq!(
            info.password, "anonymous@aimodeldownloader",
            "未指定密码应使用默认值"
        );
