...
#[cfg(test)]
use amd_core::test_harness::harness::MemoryStorage as MemStorage;
#[cfg(test)]
...
/// 分片进度回调消息
///
/// 通过 `progress_tx` 通道发送给上层(amd-app),用于:
...
impl DownloadTask {
    ...
    pub async fn new(url: String, config: DownloadConfig) -> AmdResult<Self> {
        Self::with_scheduler(
            url,
            config,
    ...
    pub async fn with_pool_and_scheduler(
        ...
        let protocol: Arc<dyn Protocol> =
            if url.starts_with("http://") || url.starts_with("https://") {
                ...
                return Err(AmdError::Config(format!("不支持的协议: {url}")));
            };
            ...
            match state {
                ...
                DownloadState::Paused => {
                    tokio::time::timeout(pause_timeout, rx.changed())
                        .await
                        .map_err(|_| {
                            AmdError::Timeout(format!("暂停超过 {} 秒", pause_timeout.as_secs()))
                        })?
    ...
    fn request_host(&self) -> AmdResult<String> {
    ...
    pub async fn probe(&mut self) -> AmdResult<&FileMetadata> {
        ...
        self.metadata
            .as_ref()
            .ok_or_else(|| AmdError::Config("探测完成但元数据未填充".into()))
    }
    ...
    async fn init_storage(&mut self) -> AmdResult<()> {
        ...
        let metadata = self
            .metadata
            .as_ref()
            .ok_or_else(|| AmdError::Config("必须先调用 probe() 获取文件元数据".into()))?;
        ...
        // 纵深防御:校验路径不逃逸下载目录
        let canonical_path = amd_core::validate_save_path(&final_path, download_dir)?;
    ...
    /// 使用调度器的带宽预测动态调整分片大小。
    /// 必须在 `probe()` 之后调用。
    pub fn plan(&mut self) -> AmdResult<Vec<FragmentInfo>> {
        let metadata = self
            .metadata
            .as_ref()
            .ok_or_else(|| AmdError::Config("必须先调用 probe() 获取文件元数据".into()))?;
    ...
    pub async fn prepare_storage(&self) -> AmdResult<()> {
        let metadata = self
            .metadata
            .as_ref()
            .ok_or_else(|| AmdError::Config("必须先调用 probe() 获取文件元数据".into()))?;
        ...
        let storage = self
            .storage
            .as_ref()
            .ok_or_else(|| AmdError::Config("存储未初始化".into()))?;
        if size > 0 {
    ...
    pub async fn execute(&mut self) -> AmdResult<()> {
        ...
        let metadata = self
            .metadata
            .as_ref()
            .ok_or_else(|| AmdError::Config("必须先调用 probe()".into()))?;
    ...
    async fn execute_full_download(&mut self) -> AmdResult<()> {
        ...
        let stream = if let Some(rx) = self.control_rx.as_mut() {
            tokio::select! {
                ...
                control = Self::watch_for_interrupt(rx, pause_timeout) => {
                    control?;
                    return Err(AmdError::Other("控制信号异常结束".into()));
                }
            }
        } else {
    ...
    async fn execute_fragmented_download(&mut self) -> AmdResult<()> {
        if self.config.max_concurrent_fragments == 0 {
            return Err(AmdError::Config(
        ...
        for (frag_index, frag_start, frag_end) in fragment_specs {
            ...
            if frag_index as usize >= self.fragments.len() {
                return Err(AmdError::Config("分片索引越界".into()));
            }
            ...
            let handle = tokio::spawn(async move {
                ...
                let permit = match frag_semaphore.acquire_owned().await {
                    ...
                    Err(e) => {
                        return Err((
                            frag_index,
                            AmdError::Other(format!("信号量获取失败: {e}").into()),
                        ));
                    }
                };
                ...
                loop {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            if matches!(e, AmdError::Cancelled | AmdError::Timeout(_)) {
                                drop(permit);
                                return Err((frag_index, e));
                            }
                            // 权限错误(401/403)重试无意义,立即终止该分片
                            if matches!(e, AmdError::Forbidden { .. }) {
                                drop(permit);
                                return Err((frag_index, e));
                            }
        ...
        for handle in handles {
            let result = handle
                .await
                .map_err(|e| AmdError::Other(format!("分片任务 panic: {e}").into()))?;
    ...
    /// 任一分片校验失败即返回 `false`。
    pub async fn verify(&mut self) -> AmdResult<()> {
    ...
    fn apply_terminal_error(&mut self, error: &AmdError) {
        if matches!(error, AmdError::Cancelled) || self.state == DownloadState::Cancelled {
...
mod tests {
    ...
    use amd_core::test_harness::harness::{test_config, test_metadata};
    use amd_core::traits::{ByteStream, Verifier as VerifierTrait};
    use bytes::Bytes;
    ...
    async fn test_probe_propagates_error() {
        let protocol = Arc::new(MockProto::failing(AmdError::Network("连接超时".into())));
        let storage = StorageKind::memory();
        let mut task = make_task(protocol, storage, test_config());
    ...
    async fn test_run_full_flow_with_mock() {
        ...
        // 调度器配置:确保恰好产生 3 个分片
        let sched_config = amd_core::config::SchedulerConfig {
    ...
    async fn test_run_failure_marks_state() {
        let protocol = Arc::new(MockProto::failing(AmdError::Network("断网".into())));
        let storage = StorageKind::memory();
        let mut task = make_task(
            protocol,
            storage,
    ...
    async fn test_concurrent_download_failure() {
        ...
        impl Protocol for FailOnSecondProtocol {
            ...
            ) -> std::pin::Pin<Box<dyn std::future::Future<Output = AmdResult<Bytes>> + Send>>
            {
                ...
                Box::pin(async move {
                    if count == 1 {
                        Err(AmdError::Network("分片 1 下载失败".into()))
                    } else {
                        Ok(data)
                    }
                })
    ...
    async fn test_fragment_retry_resilience() {
        ...
        impl Protocol for FailOnceProtocol {
            ...
            ) -> std::pin::Pin<Box<dyn std::future::Future<Output = AmdResult<Bytes>> + Send>>
            {
                ...
                Box::pin(async move {
                    if count < max_f {
                        Err(AmdError::Network(format!("模拟故障 #{}", count)))
                    } else {
                        Ok(data)
                    }
                })
    ...
    async fn test_connection_pool_permit_limits_real_range_requests() {
                ...
                Box::pin(async move {
                    ...
                    while !*release_rx.borrow() {
                        release_rx
                            .changed()
                            .await
                            .map_err(|_| AmdError::Other("释放信号关闭".into()))?;
                    }
        ...
        task.orchestrator = DownloadOrchestrator::with_scheduler_config(
            Default::default(),
            amd_core::config::SchedulerConfig {
                min_fragment_size: 100,
        ...
        fn download_range(
            ...
            Box::pin(async move {
                if start == fail_start {
                    ...
                    if n < fail_times {
                        return Err(AmdError::Network(format!("分片 {start} 模拟故障 #{n}")));
                    }
                }
    ...
    struct ClassifiedErrorProtocol {
        ...
        fail_times: u32,
        error_factory: Arc<dyn Fn() -> AmdError + Send + Sync>,
        attempts: Arc<AtomicU32>,
    }
    ...
    async fn test_forbidden_error_not_retried() {
        ...
        let protocol: Arc<dyn Protocol> = Arc::new(ClassifiedErrorProtocol {
            ...
            error_factory: Arc::new(|| AmdError::Forbidden { status: 403 }),
    ...
    async fn test_throttled_error_is_retried_and_recovers() {
        ...
        let protocol: Arc<dyn Protocol> = Arc::new(ClassifiedErrorProtocol {
            ...
            error_factory: Arc::new(|| AmdError::Throttled {
