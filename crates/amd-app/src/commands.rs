...
use amd_core::config::{AppConfig, ConnectionConfig, DownloadConfig};
use amd_core::filename::extract_filename_from_url;
use amd_core::types::DownloadState;
use amd_engine::DownloadTask;
use amd_engine::connection::{ConnectionPool, PoolConfig};
use amd_sniffer::capture::{ResourceType, identify_resource};
use chrono::Local;
use dashmap::DashMap;
...
fn validate_download_url(url_str: &str) -> Result<(), AppError> {
    ...
    amd_core::validate_public_http_url(&url).map_err(|e| AppError::Network(e.to_string()))?;
...
use amd_sniffer::SnifferResource;
...
impl AppState {
    pub fn new() -> Self {
        ...
        let store_dir = amd_core::config::dirs()
...
) -> Result<(), amd_core::AmdError> {
    loop {
        let state = *control_rx.borrow_and_update();
        match state {
            DownloadState::Cancelled => return Err(amd_core::AmdError::Cancelled),
            DownloadState::Failed => return Err(amd_core::AmdError::Other("任务已失败".into())),
            _ => control_rx
                .changed()
                .await
                .map_err(|_| amd_core::AmdError::Other("控制通道已关闭".into()))?,
        }
    }
}
...
async fn task_fn(
    ...
    match tokio::select! {
        ...
        cancel = wait_for_cancel_signal(&mut probe_cancel_rx) => {
            match cancel {
                Err(e) => Err(e),
                Ok(()) => Err(amd_core::AmdError::Other("控制信号异常结束".into())),
            }
        }
    } {
        Ok(meta) => {
        ...
        Err(amd_core::AmdError::Cancelled) => {
            cleanup_runtime(&state, &task_id);
            return;
        }
        Err(e) => {
    ...
    let (chunk_progress_tx, mut chunk_progress_rx) =
        tokio::sync::mpsc::channel::<amd_engine::FragmentProgress>(256);
...
pub async fn add_sniffer_resource(state: &AppState, url: String) {
    ...
    tracing::info!(url = %amd_core::redact_url_for_log(&url), resource_type = %resource.resource_type, "捕获新资源");
    store.push(resource);
}
...
mod tests {
    ...
    fn make_test_app_config(
        ...
        AppConfig {
            ...
            download: amd_core::config::DownloadConfig {
            ...
            scheduler: amd_core::config::SchedulerConfig::default(),
        }
    }
    ...
    fn test_build_download_config_preserves_download_fields() {
        ...
        cfg.download.user_agent = "AI Model Downloader/Custom".to_string();
        cfg.download
            .headers
        ...
        assert_eq!(download.user_agent, "AI Model Downloader/Custom");
        assert_eq!(
