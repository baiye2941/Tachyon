...
impl HubApi {
    ...
    pub async fn list_files(&self, repo_id: &str, revision: &str) -> AmdResult<Vec<HfFile>> {
        let url = lfs::build_tree_url(&self.endpoint, repo_id, revision);
        tracing::info!(url = %url, "获取 HF 仓库文件树");

        let client = reqwest::Client::new();
        let mut req = client.get(&url).header("User-Agent", "amd-hub/0.1.0");
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| amd_core::AmdError::Network(format!("HF API 请求失败: {e}")))?;

        if !resp.status().is_success() {
            return Err(amd_core::AmdError::Http {
        ...
        let body = resp
            .text()
            .await
            .map_err(|e| amd_core::AmdError::Network(format!("读取 HF API 响应失败: {e}")))?;
        let files: Vec<HfFile> =
            serde_json::from_str(&body).map_err(|e| amd_core::AmdError::Serialization(e.into()))?;
