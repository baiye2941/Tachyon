...
impl<S: AsyncStorage> WritePipeline<S> {
    ...
    pub async fn write_bytes(&self, offset: u64, data: &Bytes) -> QfResult<usize> {
        let _buf = self.buffer_pool.alloc().await;
        let written = self.storage.write_at(offset, data.as_ref()).await?;
        self.buffer_pool.release(_buf);
        Ok(written)
    }
        ...
        let total = if segments.len() == 1 {
            ...
            for &idx in &indices[1..] {
                ...
                if off == merged_end && len > 0 {
                    ...
                    // 不连续,先写出已合并的数据
                    total_written += self.storage.write_at(merged_start, &merged_data).await?;
                    // 开始新的合并组
                    merged_start = off;
                    merged_end = off + len;
                    merged_data.clear();
