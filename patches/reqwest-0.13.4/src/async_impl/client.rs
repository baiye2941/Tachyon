...
impl ClientBuilder {
    ...
    /// Set that all sockets have `SO_RCVBUF` set with the supplied size in bytes.
    ///
    /// This controls the TCP receive buffer size. On high-bandwidth high-latency
    /// networks, the default (~200KB) can become a throughput bottleneck.
    /// For example, at 1Gbps with 50ms RTT (BDP ≈ 6.25MB), setting this to
    /// 8MB (8_388_608) can significantly improve throughput.
    ///
    /// The actual buffer size may be adjusted by the OS kernel. On Linux,
    /// `net.core.rmem_max` must be large enough to accept the requested size.
    ///
    /// If `None`, the option will not be set (OS default applies).
...
impl Config {
    fn fmt_fields(&self, f: &mut fmt::DebugStruct<'_, '_>) {
        ...
        if let Some(ref v) = self.tcp_recv_buffer_size {
            f.field("tcp_recv_buffer_size", v);
        }
